"""C++ version of Parmesan-bc2026/scripts/generate_bellman_ford_cat.py.

Emits bc2026/generated/BellmanFordNavigatorCat.hpp + .cpp instead of
BellmanFordNavigatorCat.java. Same translation rules as
generate_bellman_ford.py; `Globals.catLoc` (a Java-nullable MapLocation) is the
inherited `Globals::catLoc` member, which uses the MapLocation NONE sentinel.
"""
...
### finish basic cases such as only 1 direction possible or none possible
    ...
    for dx, dy in offsets_by_distance:
        ...
        int weight{id} = canVisit{id} ? (Vision::isDirt(location{id}) ? {2 + max(abs(dx), abs(dy))} : 1) : 1000000;
            ...
            content += f"""
        int newDist{id} = location{id}.distanceSquaredTo(catLoc);
        int weight{id} = canVisit{id} ? (Vision::isDirt(location{id}) ? {2 + max(abs(dx), abs(dy))} : 1) + (currentCatDist < newDist{id} ? 0 : (currentCatDist == newDist{id} ? 2 : 4)) : 1000000;
            """.strip()
    ...
    header_file = output_dir / "BellmanFordNavigatorCat.hpp"
    with header_file.open("w+", encoding="utf-8") as file:
        file.write(HEADER)

    output_file = output_dir / "BellmanFordNavigatorCat.cpp"
