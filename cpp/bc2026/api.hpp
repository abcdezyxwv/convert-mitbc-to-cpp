// Minimal C++ shim of the Battlecode 2026 (battlecode.common) API surface.
...
enum class UnitType { BABY_RAT, RAT_KING, CAT };
inline bool isRobotType(UnitType t) { return t != UnitType::CAT; }
inline bool isThrowableType(UnitType t) { return t == UnitType::BABY_RAT; }
inline bool isThrowingType(UnitType t) { return t == UnitType::RAT_KING || t == UnitType::CAT; }
inline bool isBabyRatType(UnitType t) { return t == UnitType::BABY_RAT; }
inline bool isRatKingType(UnitType t) { return t == UnitType::RAT_KING; }
inline bool isCatType(UnitType t) { return t == UnitType::CAT; }

enum class TrapType { RAT_TRAP, CAT_TRAP, NONE };
...
struct Direction {
    ...
    int getDirectionOrderNum() const { return orderIndex[(int)e]; }
    ...
    static const Direction DIRECTION_ORDER[9];
...
private:
    static constexpr int orderIndex[9] = {3, 4, 5, 6, 7, 8, 1, 2, 0};
};

inline constexpr int Direction::orderIndex[9];
...
inline const Direction Direction::DIRECTION_ORDER[9] = {
    Direction::all[Direction::CENTER],    Direction::all[Direction::WEST],
    Direction::all[Direction::NORTHWEST], Direction::all[Direction::NORTH],
...
struct RobotInfo {
    ...
    Direction direction;
    int chirality = 0;
    UnitType type = UnitType::BABY_RAT;
    int rawCheeseAmount = 0;
    ...
    int getChirality() const { return chirality; }
    UnitType getType() const { return type; }
    int getRawCheeseAmount() const { return rawCheeseAmount; }
};

struct MapInfo {
    MapLocation loc;
    bool passable = false, wall = false, dirt = false, cheeseMine = false;
    int cheeseAmount = 0;

    bool isPassable() const { return passable; }
    bool isWall() const { return wall; }
    bool isDirt() const { return dirt; }
    bool hasCheeseMine() const { return cheeseMine; }
    int getCheeseAmount() const { return cheeseAmount; }
    MapLocation getMapLocation() const { return loc; }
};

struct Message {
    int bytes = 0, senderID = 0, round = 0;
    MapLocation sourceLoc;
    int getSenderID() const { return senderID; }
    int getRound() const { return round; }
    int getBytes() const { return bytes; }
    MapLocation getSource() const { return sourceLoc; }
};
...
public:
    ...
    bool canAttack(MapLocation loc, int cheeseAmount);
    void attack(MapLocation loc);
    void attack(MapLocation loc, int cheeseAmount);
    bool canBuildRat(MapLocation loc);
    void buildRat(MapLocation loc);
    bool canBecomeRatKing();
    void becomeRatKing();
    bool canCarryRat(MapLocation loc);
    void carryRat(MapLocation loc);
    bool canThrowRat();
    void throwRat();
    bool canPickUpCheese(MapLocation loc);
    void pickUpCheese(MapLocation loc);
    void pickUpCheese(MapLocation loc, int pickUpAmount);
    bool canTransferCheese(MapLocation loc, int amount);
    void transferCheese(MapLocation loc, int amount);
    bool canPlaceDirt(MapLocation loc);
    void placeDirt(MapLocation loc);
    bool canRemoveDirt(MapLocation loc);
    void removeDirt(MapLocation loc);
    bool canPlaceRatTrap(MapLocation loc);
    void placeRatTrap(MapLocation loc);
    bool canPlaceCatTrap(MapLocation loc);
    void placeCatTrap(MapLocation loc);
    bool squeak(int messageContent);
    vector<Message> readSqueaks(int roundNum);
